...
class TestModelFitting(unittest.TestCase):
    ...
    def test_modelFunctioning(self):
        ...
        self.assertFalse('val_loss' in history.history)
        self.assertFalse('val_accuracy' in history.history)
    ...
    def test_modelFunctioning_multipleRuns(self):
        categoricalCompiler = fnb.createCompilerDef().loss('categorical_crossentropy').optimizer('adam').metrics(['accuracy'])
        fitterDef = fnb.createFitterDef().batchSize(batchSize).epochs(nEpochs).verbose(True)
        ...
        trainedModelResults: List[fnb.TrainedModelResult] = architecture1.compilerDef(categoricalCompiler).fitterDef(fitterDef).trainSet(x_train, y_train)\
            .fitModelAndRepeat(2).ejectResults()
    ...
    def test_modelFunctioning_withChainingFitterDef(self):
        categoricalCompiler = fnb.createCompilerDef().loss('categorical_crossentropy').optimizer('adam').metrics(['accuracy'])
        ...
        trainedModelResult: fnb.TrainedModelResult = architecture1.compilerDef(categoricalCompiler).fitter() \
            ...
            .epochs(nEpochs) \
            .verbose(True) \
            .configureFitter().trainSet(x_train, y_train).fitModel().ejectResult()

        scores = trainedModelResult._model.evaluate(x_test, y_test)
        refScores = refModel.evaluate(x_test, y_test)

        self.assertEqual(len(trainedModelResult._history.history['loss']), nEpochs)
        self.assertGreater(scores[0], 0.5)
        self.assertGreater(scores[1], 0.5)
